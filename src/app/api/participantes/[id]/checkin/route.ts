...
function escolherEquipe(lista: any[]) {
  ...
  for (const p of lista) {
    if (p.equipe && counts[p.equipe] !== undefined) counts[p.equipe] += 1;
  }
...
export async function POST(_: Request, ctx: { params: { id: string } }) {
  ...
  if (participante.checkinRealizado) {
    return NextResponse.json({ message: "Check-in já realizado.", participante });
  }

  if (!participante.equipe) {
    const equipe = escolherEquipe(participantes);
    if (!equipe) {
      return NextResponse.json({ message: "Todas as equipes atingiram o limite." }, { status: 409 });
    }
  ...
  return NextResponse.json({ message: "Check-in realizado com sucesso!", participante });
}
